...
let rec repeat str n = if n = 0 then ""
                       else str ^ repeat str (n - 1)

let rec showClosure = function
  | Unit       -> "()"
  ...
  | FMul(x, y) -> x ^ " +. " ^ y
  | FDiv(x, y) -> x ^ " -. " ^ y
  | IfEq(x, y, e1, e2)  -> "if " ^ x ^ " = " ^ y ^ " then " ^ showClosure e1 ^ " else " ^ showClosure e2
  | IfLE(x, y, e1, e2)  -> "if " ^ x ^ " < " ^ y ^ " then " ^ showClosure e1 ^ " else " ^ showClosure e2
  | Let((x, t), e1, e2) -> "let " ^ x ^ " = " ^ showClosure e1 ^ " in\n" ^ showClosure e2
  | Var(x) -> x
  | MakeCls((x, t), { entry = l; actual_fv = ys }, e) -> x ^ "(" ^ showClosure e ^ ")"
  | AppCls(x, ys) -> "(AppCls " ^ x ^ ")"
  | AppDir(L(x), ys) -> "AppDir :: " ^ x ^ "(" ^ showTuple ys ^ ")"
  | Tuple(xs) -> "(" ^ (showTuple xs) ^ ")"
  | LetTuple(xts, y, e) -> "(LetTuple)"
  | Get(x, y) -> x ^ ".(" ^ y ^ ")"
  | Put(x, y, z) -> x ^ ".(" ^ y ^ ") <- " ^ z
  | ExtArray(l) -> "(ExtArray)"
