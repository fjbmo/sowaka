...
and g' oc = function (* 各命令のアセンブリ生成 *)
  ...
  | (NonTail(x), Swl(y, V(z))) -> 
      Printf.fprintf oc "\tswl\t%s, %s, %s\n" (reg x) (reg y) (reg z)
  | (NonTail(x), Swl(y, C(z))) -> 
      Printf.fprintf oc "\tswli\t%s, %s, %d\n" (reg x) (reg y) z
  | (NonTail(x), Lw(y, V(z))) ->
...
and g'_args oc x_reg_cl ys zs = 
  let (i, yrs) = List.fold_left (fun (i, yrs) y -> (i + 1, (y, regs.(i)) :: yrs)) (0, x_reg_cl) ys in
  List.iter (fun (y, r) -> Printf.fprintf oc "\tmr\t%s, %s\n" (reg r) (reg y)) (shuffle reg_sw yrs);
  let (d, zfrs) = List.fold_left (fun (d, zfrs) z -> (d + 1, (z, fregs.(d)) :: zfrs)) (0, []) zs in
  List.iter (fun (z, fr) -> Printf.fprintf oc "\tfmr\t%s, %s\n" (reg fr) (reg z)) (shuffle reg_fsw zfrs)
