...
let rec printer = function
  ...
  | Float(b) -> "Float (" ^ string_of_float b ^ ")"
  | Var(s) -> s
  | Not(e) -> "Not (" ^ printer e ^ ")"
  | Neg(e) -> "Neg (" ^ printer e ^ ")"
  | Add(e1, e2) -> "Add (" ^ printer e1 ^ ", " ^ printer e2 ^ ")"
  | Sub(e1, e2) -> "Sub (" ^ printer e1 ^ ", " ^ printer e2 ^ ")"
  | Mul(e1, e2) -> "Mul (" ^ printer e1 ^ ", " ^ printer e2 ^ ")"
  | Div(e1, e2) -> "Div (" ^ printer e1 ^ ", " ^ printer e2 ^ ")"
  | Eq(e1, e2) -> "Eq (" ^ printer e1 ^ ", " ^ printer e2 ^ ")"
  | LE(e1, e2) -> "LE (" ^ printer e1 ^ ", " ^ printer e2 ^ ")"
  | FNeg(e) -> "FNeg (" ^ printer e ^ ")"
  | FAdd(e1, e2) -> "FAdd (" ^ printer e1 ^ ", " ^ printer e2 ^ ")"
  | FSub(e1, e2) -> "FSub (" ^ printer e1 ^ ", " ^ printer e2 ^ ")"
  | FMul(e1, e2) -> "FMul (" ^ printer e1 ^ ", " ^ printer e2 ^ ")"
  | FDiv(e1, e2) -> "FDiv (" ^ printer e1 ^ ", " ^ printer e2 ^ ")"
  | If(e1, e2, e3) -> "If " ^ printer e1 ^ " THEN " ^ printer e2 ^ " ELSE " ^ printer e3
  | Let((i, t), e1, e2) -> "LET (" ^ i ^ ", " ^ printer e2 ^ ")"
  | LetRec({ name = xt; args = yts; body = e1 }, e2) -> "LETREC (" ^ printer e1 ^ ", " ^ printer e2 ^ ")"
  | App(e, es) -> "App (" ^ printer e ^ ")"
  | Tuple(es) -> "Tuple (pohe)"
  | LetTuple(xts, e1, e2) -> "LetTuple (pohe)"
  | Array(e1, e2) -> "Array(pohe)"
  | Get(e1, e2) -> "Get (" ^ printer e1 ^ ", " ^ printer e2 ^ ")"
  | Put(e1, e2, e3) -> "Put (" ^ printer e1 ^ ", " ^ printer e2 ^ ", " ^ printer e3 ^ ")"
  | e -> "any"
...
let rec unify t1 t2 = (* 型が合うように、型変数への代入をする (caml2html: typing_unify) *)
  ...
  | Type.Fun(t1s, t1'), Type.Fun(t2s, t2') ->
      Printf.printf "try fun...\n";
      (try List.iter2 unify t1s t2s; Printf.printf "---\n"
      with Invalid_argument("List.iter2") -> raise (Unify(t1, t2)));
      unify t1' t2';
      Printf.printf "fun ok.\n";
  | Type.Tuple(t1s), Type.Tuple(t2s) ->
      Printf.printf "try (tuple)...\n";
      (try List.iter2 unify t1s t2s; Printf.printf "---\n"
      with Invalid_argument("List.iter2") -> raise (Unify(t1, t2)));
      Printf.printf "tuple ok.\n";
